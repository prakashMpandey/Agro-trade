...
const changePassword = asyncHandler(async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(500)
        .json(new ApiError(500, "password cannot be changed"));
    }
    ...
    return res
      .status(500)
      .json(new ApiError(500, "password cannot be changed", error));
  }
});
...
export const addContactDetails = async (req, res) => {
  ...
  if (
    !(country || state || city || pinCode || street || landmark || addressType)
  ) {
    return res
      .status(400)
      .json(new ApiError(400, "At least one field is required"));
  }
...
export const addAvatar = async (req, res) => {
  try {
    ...
    const user = await User.findByIdAndUpdate(
      req.user._id,
      { $set: { avatar: AvatarcloudinaryUrl?.url || null } },
      { new: true }
    );
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, avatar, "avatar uploaded successfullly"));
  } catch (error) {
    return res
      .status(400)
      .json(
        new ApiError(400, "an error occured while uploading avatar", error)
      );
  }
};
...
const forgotPassword = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json(
        new ApiResponse(200, resetURL, "reset password url sent successfully")
      );
  } catch (error) {
    return res
      .status(400)
      .json(new ApiError(400, "reset link cannot be sent", error));
  }
};
...
const resetPassword = async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(400)
        .json(new ApiError(400, "cannot reset password, link expired!!"));
    }
...
const updateProfile = async (req, res) => {
  try {
    ...
    if (city) contactUpdateData.city = city;
